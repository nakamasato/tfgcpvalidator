...
3. Release の published を受けて `release.yml` が goreleaser を走らせ、
...
3 が別ワークフローとして成立するのは、release-please を App トークンで
動かしているからである。`GITHUB_TOKEN` による操作は `workflow_dispatch` と
`repository_dispatch` を除き新しいワークフロー実行を起こさないため、
トークンを `GITHUB_TOKEN` に戻すと `release: published` が発火せず、
**バイナリが永久に添付されない**。トークンの選択とワークフローの分離は
セットの判断である。

分離しておく利点は 2 つある。goreleaser だけが失敗した場合に release-please を
巻き戻さずに再実行できること、そして手で切った Release でも発火することである。
...
誰かが `v0` を動かさない限りその参照は壊れたままになる。移動タグはタグ名から
算出するため、`release.yml` は算出前にタグ形式を検証し、想定外なら失敗する。
